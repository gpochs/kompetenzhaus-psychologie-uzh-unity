import { createFrameworkView, moduleLearningProposal } from './framework-view.mjs';
...
function renderHud() {
  ...
  const hint = state.interactionHint || (exploring ? (state.language === 'en' ? 'Press E near a learning place. Escape releases the mouse.' : 'E am Lernort · Escape gibt die Maus frei.') : (state.language === 'en' ? 'Click a learning place to start. Design your house in the building studio.' : 'Lernort anklicken und loslegen. Im Bauatelier gestaltest du dein Haus.'));
...
function renderModuleProposal(body, slot, module) {
  const version = state?.frameworkProfile?.frameworkVersion || '2.1.0-draft';
...
const frameworkView = createFrameworkView({ onModule(id) { closeAllDialogs(); command('startModuleQuiz', { moduleId: id }); } });
